...
if (user.profile_link) profileImg.src = 'img/' + user.profile_link
...
function videosRenderer(array, username, profile_link) {
	...
	array.map(video => {
		...
		<video class="div-video" src=${'/vid/' + video.video_link} controls></video>
		<div class="wrap">
		<img class="img-videos" src=${'/img/' + video.user.profile_link} alt="blog">
...
function usersRenderer(array) {
	...
	array.map(user => {
		...
		<img src=${'img/' + user.profile_link} alt="channel-icon" width="30px" height="30px" class"img-following-pages">
